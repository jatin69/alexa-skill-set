...
var handlers = {
  'LaunchRequest': function() {
    ...
    .listen("Tell me what you think is Codecademy's most popular language.");
    this.emit(':responseReady');
  },

    "helloIntent" : function() {
      this.response.speak("hey Jatin, Lets play the game of languages.").listen();
      this.emit(':responseReady');
    },
  'LanguageIntent': function () {
    var myLanguage = this.event.request.intent.slots.language.value;
    if (myLanguage == undefined){
        this.response.speak("No problem boss ! Python is our most popular language");
    }
    else if (myLanguage == "python") {
