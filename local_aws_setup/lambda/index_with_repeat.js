...
var playModeHandlers = Alexa.CreateStateHandler(states.PLAYMODE, {
    ...
    'CityChainIntent': function () {
        ...
        request(validateUserInputOptions, function (error, response, body) {
            ...
            // fetch the address components from api response of => 'user string'
            var userCity = _.find(jsonUserCityResponse.results[0].address_components, 
                function (o) { 
                    return _.includes(o.types, 'locality' ); 
                });

            if (userCity && (userCity.long_name.toLowerCase().indexOf(city.toLowerCase()) >= 0 ||
             city.toLowerCase().indexOf(userCity.long_name.toLowerCase()) >= 0)) {
